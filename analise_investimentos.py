...
# Função para recomendar a compra ou não e exibir a projeção de crescimento
def recomendar_compra_e_projecao(ticker, df):
    ...
    if tendencia == "Crescimento":
        st.success(f"{seta} **Recomendação:** {ticker} está em **Crescimento** ({percentual:.2f}% de crescimento).")
    else:
        st.error(f"{seta} **Recomendação:** {ticker} está em **Queda** ({percentual:.2f}% de queda).")

    # Cálculo de projeção de crescimento
    crescimento_projetado = calcular_projecao_crescimento(df)
    st.write(f"**Crescimento projetado para os próximos dias**: {crescimento_projetado:.2f}%")
...
def main():
    ...
    Agora, também calculamos uma **projeção de crescimento futuro** baseada no momento atual de crescimento e volatilidade.
    """)
    ...
    if st.sidebar.button("Analisar"):
        ...
        for ticker in tickers_acoes:
            ...
            # Recomendação de compra e projeção de crescimento
